...
export const GlobalEvent = {
    ...
    off(comp, type) {
        DUtils.remove(eventStore, item => item.comp === comp && item.type === type)
    },
    trigger(evnt) {
...
if (browse.isDoc) {
    ...
    window.addEventListener(wheelName, DUtils.throttle(GlobalEvent.trigger, 100, { leading: true, trailing: false }), false)
}