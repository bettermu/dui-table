...
export const browse = DUtils.browse()
const reClsMap = {}
...
export function getPaddingTopBottomSize(elem) {
    if (elem) {
        ...
        const paddingTop = DUtils.toNumber(computedStyle.paddingTop)
        const paddingBottom = DUtils.toNumber(computedStyle.paddingBottom)
    ...
    calcHeight($xetable, key) {
        ...
        if (val) {
            if (val === 'auto') {
                ...
                if (isScale(val)) {
                    num = Math.floor((DUtils.toInteger(val) || 1) / 100 * $xetable.parentHeight)
                } else {
                    num = DUtils.toNumber(val)
                }
