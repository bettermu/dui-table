...
import { getRowid, getRowkey, clearTableAllStatus, handleFieldOrColumn, restoreScrollLocation, restoreScrollListener, toTreePathSeq } from './utils'
...
function getRowUniqueId() {
    return DUtils.uniqueId('row_')
}

function eqCellValue(row1, row2, field) {
    const val1 = DUtils.get(row1, field)
    const val2 = DUtils.get(row2, field)
    if (eqEmptyValue(val1) && eqEmptyValue(val2)) {
        return true
    }
    if (DUtils.isString(val1) || DUtils.isNumber(val1)) {
        /* eslint-disable eqeqeq */
        return val1 == val2
    }
    return DUtils.isEqual(val1, val2)
}
...
function getCustomStorageMap(key) {
    ...
    const rest = DUtils.toStringJSON(localStorage.getItem(key))
...
function handleReserveRow(_vm, reserveRowMap) {
    ...
    const reserveList = []
    DUtils.each(reserveRowMap, (item, rowid) => {
...
function setMerges(_vm, merges, mList, rowList) {
    if (merges) {
        ...
        if (!DUtils.isArray(merges)) {
            merges = [merges]
        }
        merges.forEach(item => {
            let { row, col, rowspan, colspan } = item
            if (rowList && DUtils.isNumber(row)) {
                row = rowList[row]
            }
            if (DUtils.isNumber(col)) {
                col = visibleColumn[col]
            }
            if ((rowList ? row : DUtils.isNumber(row)) && col && (rowspan || colspan)) {
                rowspan = DUtils.toNumber(rowspan) || 1
                colspan = DUtils.toNumber(colspan) || 1
                if (rowspan > 1 || colspan > 1) {
                    const mcIndex = DUtils.findIndexOf(mList, item => item._row === row && item._col === col)
...
function getOrderField(_vm, column) {
    ...
    return (row) => {
        let cellValue
        if (sortBy) {
            cellValue = DUtils.isFunction(sortBy) ? sortBy({ row, column }) : DUtils.get(row, sortBy)
        } else {
            cellValue = _vm.getCellLabel(row, column)
        }
        if (!sortType || sortType === 'auto') {
            return isNaN(cellValue) ? cellValue : DUtils.toNumber(cellValue)
        } else if (sortType === 'number') {
            return DUtils.toNumber(cellValue)
        } else if (sortType === 'string') {
            return DUtils.toValueString(cellValue)
        }
        return cellValue
    }
}
...
const Methods = {
    ...
    loadTableData(datas) {
        ...
        if (treeConfig) {
            ...
            if (treeOpts.transform) {
                ...
                treeData = DUtils.toArrayTree(fullData, {
        ...
        if (keepSource) {
            this.tableSourceData = DUtils.clone(fullData, true)
        }
        if (process.env.VUE_APP_VXE_TABLE_ENV === 'development') {
            if (sYLoad) {
    ...
    reloadRow(row, record, field) {
        ...
        if (keepSource) {
            ...
            if (oRow && row) {
                if (field) {
                    const newValue = DUtils.get(record || row, field)
                    DUtils.set(row, field, newValue)
                    DUtils.set(oRow, field, newValue)
                } else {
                    const newRecord = DUtils.clone({...record }, true)
                    DUtils.destructuring(oRow, Object.assign(row, newRecord))
                }
            }
     ...
     */
    loadColumn(columns) {
        const collectColumn = DUtils.mapTree(columns, column => Cell.createColumn(this, column), { children: 'children' })
    ...
    cacheRowMap(source) {
        ...
        const handleCache = (row, index, items, path, parent, nodes) => {
            ...
            if (eqEmptyValue(rowid)) {
                ...
                DUtils.set(row, rowkey, rowid)
            }
            if (isLazy && row[treeOpts.hasChild] && DUtils.isUndefined(row[treeOpts.children])) {
        ...
        if (treeConfig) {
            DUtils.eachTree(tableFullTreeData, handleCache, treeOpts)
        } else {
    ...
    loadTreeChildren(row, childRecords) {
        ...
        return this.createData(childRecords).then((rows) => {
            if (keepSource) {
                const rowid = getRowid(this, row)
                const matchObj = DUtils.findTree(tableSourceData, (item) => rowid === getRowid(this, item), treeOpts)
                if (matchObj) {
                    matchObj.item[children] = DUtils.clone(rows, true)
                }
            }
            DUtils.eachTree(rows, (childRow, index, items, path, parent, nodes) => {
    ...
    defineField(record) {
        ...
        this.tableFullColumn.forEach(column => {
            ...
            if (property && !DUtils.has(record, property)) {
                let cellValue = null
                if (editRender) {
                    const { defaultValue } = editRender
                    if (DUtils.isFunction(defaultValue)) {
                        cellValue = defaultValue({ column })
                    } else if (!DUtils.isUndefined(defaultValue)) {
                        cellValue = defaultValue
                    }
                }
                DUtils.set(record, property, cellValue)
            }
        })
        const otherFields = [radioOpts.labelField, checkboxOpts.checkField, checkboxOpts.labelField, expandOpts.labelField]
        otherFields.forEach((key) => {
            if (key && eqEmptyValue(DUtils.get(record, key))) {
                DUtils.set(record, key, null)
            }
        })
        if (treeConfig && treeOpts.lazy && DUtils.isUndefined(record[treeOpts.children])) {
            record[treeOpts.children] = null
        }
        // 必须有行数据的唯一主键，可以自行设置；也可以默认生成一个随机数
        if (eqEmptyValue(DUtils.get(record, rowkey))) {
            DUtils.set(record, rowkey, getRowUniqueId())
        }
        return record
    },
    /**
    ...
    createData(records) {
        ...
        const rows = treeConfig ? DUtils.mapTree(records, handleRrecord, treeOpts) : records.map(handleRrecord)
     ...
     */
    createRow(records) {
        const isArr = DUtils.isArray(records)
        if (!isArr) {
            records = [records]
        }
    ...
    revertData(rows, field) {
        ...
        let targetRows = rows
        if (rows) {
            if (!DUtils.isArray(rows)) {
                targetRows = [rows]
            }
        } else {
            targetRows = DUtils.toArray(this.getUpdateRecords())
        }
        if (targetRows.length) {
            targetRows.forEach(row => {
                if (!this.isInsertByRow(row)) {
                    ...
                    if (oRow && row) {
                        if (field) {
                            DUtils.set(row, field, DUtils.clone(DUtils.get(oRow, field), true))
                        } else {
                            DUtils.destructuring(row, DUtils.clone(oRow, true))
                        }
                    }
                }
            })
    ...
    clearData(rows, field) {
        ...
        if (!arguments.length) {
            rows = tableFullData
        } else if (rows && !DUtils.isArray(rows)) {
            rows = [rows]
        }
        if (field) {
            rows.forEach(row => DUtils.set(row, field, null))
        } else {
            rows.forEach(row => {
    ...
    getColumns(columnIndex) {
        ...
        return DUtils.isUndefined(columnIndex) ? columns.slice(0) : columns[columnIndex]
    },
    /**
     * 根据列的唯一主键获取列
    ...
    getData(rowIndex) {
        ...
        return DUtils.isUndefined(rowIndex) ? tableSynchData.slice(0) : tableSynchData[rowIndex]
    },
    /**
     * 用于多选行，获取已选中的数据
     */
    getCheckboxRecords(isFull) {
        ...
        if (property) {
            if (treeConfig) {
                rowList = DUtils.filterTree(currTableData, row => DUtils.get(row, property), { children: transform ? mapChildren : children })
            } else {
                rowList = currTableData.filter(row => DUtils.get(row, property))
            }
        } else {
            const { selection } = this
            if (treeConfig) {
                rowList = DUtils.filterTree(currTableData, row => selection.indexOf(row) > -1, { children: transform ? mapChildren : children })
            } else {
    ...
    handleVirtualTreeToList() {
        ...
        if (treeConfig && treeOpts.transform) {
            ...
            DUtils.eachTree(afterTreeFullData, (row, index, items, path, parent) => {
    ...
    updateAfterFullData() {
        ...
        if (filterColumns.length) {
            const handleFilter = (row) => {
                return filterColumns.every(({ column, valueList, itemList }) => {
                    if (valueList.length && !allRemoteFilter) {
                        ...
                        return valueList.indexOf(DUtils.get(row, column.property)) > -1
                    }
                    return true
                })
            }
            if (treeConfig && transform) {
                // 筛选虚拟树
                tableTree = DUtils.searchTree(tableFullTreeData, handleFilter, {...treeOpts, original: true })
                tableData = tableTree
            } else {
            ...
            if (treeConfig && transform) {
                // 还原虚拟树
                tableTree = DUtils.searchTree(tableFullTreeData, () => true, {...treeOpts, original: true })
                tableData = tableTree
            } else {
        ...
        if (!allRemoteSort && firstOrderColumn) {
            if (treeConfig && transform) {
                ...
                if (allSortMethod) {
                    ...
                    tableTree = DUtils.isArray(sortRests) ? sortRests : tableTree
                } else {
                    tableTree = DUtils.orderBy(tableTree, orderColumns.map(({ column, order }) => [getOrderField(this, column), order]))
                }
                tableData = tableTree
            } else {
                if (allSortMethod) {
                    ...
                    tableData = DUtils.isArray(sortRests) ? sortRests : tableData
                } else {
                    // 兼容 v4
                    if (sortMultiple) {
                        tableData = DUtils.orderBy(tableData, orderColumns.map(({ column, order }) => [getOrderField(this, column), order]))
                    } else {
                        // 兼容 v2，在 v4 中废弃， sortBy 不能为数组
                        let sortByConfs
                        if (DUtils.isArray(firstOrderColumn.sortBy)) {
                            sortByConfs = firstOrderColumn.sortBy.map(item => [item, firstOrderColumn.order])
                        }
                        tableData = DUtils.orderBy(tableData, sortByConfs || [firstOrderColumn].map(({ column, order }) => [getOrderField(this, column), order]))
                    }
                }
                tableTree = tableData
            }
    ...
    updateAfterDataIndex() {
        ...
        if (treeConfig) {
            DUtils.eachTree(afterTreeFullData, (row, index, items, path) => {
                const rowid = getRowid(this, row)
    ...
    saveCustomVisible() {
        ...
        if (customConfig && isVisible) {
            ...
            localStorage.setItem(visibleStorageKey, DUtils.toJSONString(columnVisibleStorageMap))
        }
    },
    saveCustomResizable(isReset) {
        ...
        if (customConfig && isResizable) {
            ...
            if (!isReset) {
                columnWidthStorage = DUtils.isPlainObject(columnWidthStorageMap[id]) ? columnWidthStorageMap[id] : {}
                DUtils.eachTree(collectColumn, column => {
            ...
            columnWidthStorageMap[id] = DUtils.isEmpty(columnWidthStorage) ? undefined : columnWidthStorage
            localStorage.setItem(resizableStorageKey, DUtils.toJSONString(columnWidthStorageMap))
        }
    },
    /**
     * 刷新列配置
                ...
                if (operCtxMenu) {
                    ...
                    if (!isEditStatus) {
                        ...
                        if (keyboardOpts.isDel && (selected.row || selected.column)) {
                            ...
                            // 如果树形表格回退键关闭当前行返回父节点
                            const { parent: parentRow } = DUtils.findTree(this.afterFullData, item => item === currentRow, treeOpts)
                            if (parentRow) {
                                evnt.preventDefault()
    ...
    handleTooltip(evnt, cell, overflowElem, tipElem, params) {
        ...
        const useCustom = contentMethod && !DUtils.eqNull(customContent)
    ...
    isCheckedByCheckboxRow(row) {
        ...
        if (property) {
            return DUtils.get(row, property)
        }
        ...
        if (property) {
            if (treeConfig && !checkStrictly) {
                if (value === -1) {
                    ...
                    DUtils.set(row, property, false)
                } else {
                    // 更新子节点状态
                    DUtils.eachTree([row], (item) => {
                        if (row === item || (!checkMethod || checkMethod({ row: item }))) {
                            DUtils.set(item, property, value)
                            DUtils.remove(treeIndeterminates, half => half === item)
                            this.handleCheckboxReserveRow(row, value)
                        }
                    }, treeOpts)
                }
                // 如果存在父节点，更新父节点状态
                const matchObj = DUtils.findTree(afterFullData, item => item === row, treeOpts)
                if (matchObj && matchObj.parent) {
                    let parentStatus
                    const vItems = checkMethod ? matchObj.items.filter((item) => checkMethod({ row: item })) : matchObj.items
                    const indeterminatesItem = DUtils.find(matchObj.items, item => treeIndeterminates.indexOf(item) > -1)
                    if (indeterminatesItem) {
                        parentStatus = -1
                    } else {
                        const selectItems = matchObj.items.filter(item => DUtils.get(item, property))
            ...
            if (treeConfig && !checkStrictly) {
                if (value === -1) {
                    ...
                    DUtils.remove(selection, item => item === row)
                } else {
                    // 更新子节点状态
                    DUtils.eachTree([row], (item) => {
                        if (row === item || (!checkMethod || checkMethod({ row: item }))) {
                            if (value) {
                                ...
                                DUtils.remove(selection, select => select === item)
                            }
                            DUtils.remove(treeIndeterminates, half => half === item)
                            this.handleCheckboxReserveRow(row, value)
                        }
    ...
    handleToggleCheckRowEvent(evnt, params) {
        ...
        const { checkField: property } = checkboxOpts
        const { row } = params
        const value = property ? !DUtils.get(row, property) : selection.indexOf(row) === -1
        if (evnt) {
        ...
        if (checkStrictly) {
            ...
            if (property) {
                const checkValFn = (row) => {
                    ...
                    // 如果方法成立，则更新值，否则忽略该数据
                if (treeConfig) {
                    DUtils.eachTree(afterFullData, checkValFn, treeOpts)
                } else {
                ...
                if (treeConfig) {
                    if (value) {
                         ...
                         * 如果方法成立，则添加到临时集合中
                         */
                        DUtils.eachTree(afterFullData, (row) => {
                            if (!checkMethod || checkMethod({ row })) {
                                selectRows.push(row)
                            }
                        }, treeOpts)
            ...
            if (checkField) {
                isAllResolve = afterFullData.every(
                    ...
                    (row) => {
                        ...
                        if (DUtils.get(row, checkField)) {
                            checkRows.push(row)
                            return true
                        }
                        return false
                    } :
                    row => DUtils.get(row, checkField)
                )
                isAllSelected = isAllResolve && afterFullData.length !== disableRows.length
                if (treeConfig) {
                    if (halfField) {
                        isIndeterminate = !isAllSelected && afterFullData.some(row => DUtils.get(row, checkField) || DUtils.get(row, halfField) || treeIndeterminates.indexOf(row) > -1)
                    } else {
                        isIndeterminate = !isAllSelected && afterFullData.some(row => DUtils.get(row, checkField) || treeIndeterminates.indexOf(row) > -1)
                    }
                } else {
                    if (halfField) {
                        isIndeterminate = !isAllSelected && afterFullData.some(row => DUtils.get(row, checkField) || DUtils.get(row, halfField))
                    } else {
                        isIndeterminate = !isAllSelected && afterFullData.some(row => DUtils.get(row, checkField))
                    }
                }
            } else {
                isAllResolve = afterFullData.every(
    ...
    getRadioReserveRecord(isFull) {
        ...
        if (radioOpts.reserve && radioReserveRow) {
            ...
            if (isFull) {
                ...
                const rowkey = getRowkey(this)
                if (treeConfig) {
                    const matchObj = DUtils.findTree(afterFullData, row => rowid === DUtils.get(row, rowkey), treeOpts)
                    if (matchObj) {
                        return radioReserveRow
                    }
                } else {
                    if (!afterFullData.some(row => rowid === DUtils.get(row, rowkey))) {
                        return radioReserveRow
                    }
                }
            }
    ...
    getCheckboxReserveRecords(isFull) {
        ...
        if (checkboxOpts.reserve) {
            ...
            DUtils.each(checkboxReserveRowMap, (oldRow, oldRowid) => {
                if (oldRow) {
                    if (isFull) {
    ...
    clearCheckboxRow() {
        ...
        if (property) {
            if (treeConfig) {
                DUtils.eachTree(tableFullData, item => DUtils.set(item, property, false), treeOpts)
            } else {
                tableFullData.forEach(item => DUtils.set(item, property, false))
            }
        }
        if (reserve) {
    ...
    setCurrentRow(row) {
        ...
        if (rowOpts.isCurrent || this.highlightCurrentRow) {
            if ($el) {
                DUtils.arrayEach($el.querySelectorAll(`[rowid="${getRowid(this, row)}"]`), elem => addClass(elem, 'row--current'))
            }
        }
        return this.$nextTick()
    },
    ...
    clearCurrentRow() {
        ...
        this.hoverRow = null
        if ($el) {
            DUtils.arrayEach($el.querySelectorAll('.row--current'), elem => removeClass(elem, 'row--current'))
        }
        return this.$nextTick()
    },
    /**
    ...
    setHoverRow(row) {
        ...
        this.clearHoverRow()
        if ($el) {
            DUtils.arrayEach($el.querySelectorAll(`[rowid="${rowid}"]`), elem => addClass(elem, 'row--hover'))
        }
        this.hoverRow = row
    },
    clearHoverRow() {
    ...
    sort(sortConfs, sortOrder) {
        ...
        if (sortConfs) {
            if (DUtils.isString(sortConfs)) {
                sortConfs = [
                    { field: sortConfs, order: sortOrder }
                ]
            }
        }
        if (!DUtils.isArray(sortConfs)) {
            sortConfs = [sortConfs]
        }
        if (sortConfs.length) {
            let firstSortColumn
    ...
    getSortColumn() {
        ...
        return DUtils.find(this.tableFullColumn, column => (column.sortable || column.remoteSort) && column.order)
    },
    ...
    clearRowExpandLoaded(row) {
        ...
        if (lazy && rest) {
            ...
            DUtils.remove(expandLazyLoadeds, item => row === item)
        }
        return this.$nextTick()
    },
    /**
    ...
    handleAsyncRowExpand(row) {
        ...
        return new Promise(resolve => {
            ...
            this.expandOpts.loadMethod({ $table: this, row, rowIndex: this.getRowIndex(row), $rowIndex: this.getVMRowIndex(row) }).catch(e => e).then(() => {
                ...
                DUtils.remove(this.expandLazyLoadeds, item => item === row)
    ...
    setRowExpand(rows, expanded) {
        ...
        if (rows) {
            ...
            if (expanded) {
                ...
                DUtils.remove(rowExpandeds, row => validRows.indexOf(row) > -1)
            }
            if (reserve) {
    ...
    clearTreeExpandLoaded(row) {
        ...
        if (lazy && rest) {
            ...
            DUtils.remove(treeExpandeds, item => row === item)
        }
        if (transform) {
    ...
    handleAsyncTreeExpandChilds(row) {
        ...
        return new Promise(resolve => {
            ...
            loadMethod({ $table: this, row }).catch(() => []).then(childRecords => {
                ...
                DUtils.remove(treeLazyLoadeds, item => item === row)
                if (!DUtils.isArray(childRecords)) {
                    childRecords = []
                }
                if (childRecords) {
    ...
    setAllTreeExpand(expanded) {
        ...
        const expandeds = []
        DUtils.eachTree(tableFullData, row => {
    ...
    handleBaseTreeExpand(rows, expanded) {
        ...
        if (accordion) {
            validRows = validRows.length ? [validRows[validRows.length - 1]] : []
                // 同一级只能展开一个
            const matchObj = DUtils.findTree(tableFullData, item => item === validRows[0], treeOpts)
            if (matchObj) {
                DUtils.remove(treeExpandeds, item => matchObj.items.indexOf(item) > -1)
            }
        }
        if (expanded) {
            validRows.forEach(row => {
            ...
            DUtils.remove(treeExpandeds, row => validRows.indexOf(row) > -1)
        }
        if (reserve) {
    ...
    clearTreeExpand() {
        ...
        this.treeExpandeds = []
        if (reserve) {
            DUtils.eachTree(tableFullData, row => this.handleTreeExpandReserve(row, false), treeOpts)
        }
    ...
    debounceScrollY: DUtils.debounce(function(evnt) {
        this.loadScrollYData(evnt)
    ...
    scrollTo(scrollLeft, scrollTop) {
        ...
        const rightBodyElem = rightBody ? rightBody.$el : null
        const tableFooterElem = tableFooter ? tableFooter.$el : null
        if (DUtils.isNumber(scrollLeft)) {
            setScrollLeft(tableFooterElem || tableBodyElem, scrollLeft)
        }
        if (DUtils.isNumber(scrollTop)) {
    ...
    scrollToTreeRow(row) {
        ...
        const rests = []
        if (treeConfig) {
            const matchObj = DUtils.findTree(tableFullData, item => item === row, treeOpts)
            if (matchObj) {
