...
function parseDate(value, props) {
    return value && props.valueFormat ? DUtils.toStringDate(value, props.valueFormat) : value
}

function getFormatDate(value, props, defaultFormat) {
    const { dateConfig = {} } = props
    return DUtils.toDateString(parseDate(value, props), dateConfig.labelFormat || defaultFormat)
}
...
function getCellEditProps(renderOpts, params, value, defaultProps) {
    ...
    return DUtils.assign({ immediate: getInputImmediateModel(renderOpts) }, vSize ? { size: vSize } : {}, defaultCompProps, defaultProps, renderOpts.props, {
...
function getFilterProps(renderOpts, params, value, defaultProps) {
    const { vSize } = params.$table
    return DUtils.assign(vSize ? { size: vSize } : {}, defaultCompProps, defaultProps, renderOpts.props, {
        [componentDefaultModelProp]: value
    })
}
...
function getNativeOns(renderOpts, params) {
    ...
    const nativeOns = {}
    DUtils.objectEach(nativeEvents, (func, key) => {
...
function getOns(renderOpts, params, inputFunc, changeFunc) {
    ...
    const ons = {}
    DUtils.objectEach(events, (func, key) => {
...
function getFilterOns(renderOpts, params, option) {
    return getOns(renderOpts, params, (value) => {
        ...
        handleConfirmFilter(params, !DUtils.eqNull(option.data), option)
    })
}
...
function getItemOns(renderOpts, params) {
    ...
    return getOns(renderOpts, params, (value) => {
        // 处理 model 值双向绑定
        DUtils.set(data, property, value)
    }, () => {
        // 处理 change 事件相关逻辑
...
function getNativeItemOns(renderOpts, params) {
    ...
    return getOns(renderOpts, params, (evnt) => {
        ...
        DUtils.set(data, property, itemValue)
    }, () => {
        // 处理 change 事件相关逻辑
        $form.updateStatus(params)
    })
...
function handleFilterMethod({ option, row, column }) {
    const { data } = option
    const cellValue = DUtils.get(row, column.property)
        /* eslint-disable eqeqeq */
...
function getSelectCellValue(renderOpts, { row, column }) {
    ...
    if (!isEmptyValue(cellValue)) {
        return DUtils.map(props.multiple ? cellValue : [cellValue], optionGroups ? (value) => {
            const groupOptions = optionGroupProps.options || 'options'
            for (let index = 0; index < optionGroups.length; index++) {
                /* eslint-disable eqeqeq */
                selectItem = DUtils.find(optionGroups[index][groupOptions], item => item[valueProp] == value)
                if (selectItem) {
                    break
                }
            }
            ...
            /* eslint-disable eqeqeq */
            selectItem = DUtils.find(options, item => item[valueProp] == value)
            return selectItem ? selectItem[labelProp] : value
        }).join(', ')
    }
    return null
...
function nativeItemRender(h, renderOpts, params) {
    ...
    const { name } = renderOpts
    const attrs = getNativeAttrs(renderOpts)
    const itemValue = DUtils.get(data, property)
    return [
        h(name, {
            class: `vxe-default-${name}`,
            attrs,
...
function renderNativeFormOptions(h, options, renderOpts, params) {
    ...
    const valueProp = optionProps.value || 'value'
    const disabledProp = optionProps.disabled || 'disabled'
    const cellValue = DUtils.get(data, property)
...
const renderMap = {
    ...
    $input: {
        ...
        renderCell(h, renderOpts, params) {
            ...
            let cellValue = DUtils.get(row, column.property)
            if (cellValue) {
                switch (props.type) {
                    case 'date':
                    case 'week':
                    ...
                    case 'year':
                        ...
                        break
                    case 'float':
                        cellValue = DUtils.toFixed(DUtils.floor(cellValue, digits), digits)
                        break
                }
            }
...
export const renderer = {
    mixin(map) {
        DUtils.each(map, (options, name) => renderer.add(name, options))
        return renderer
    },
    get(name) {
